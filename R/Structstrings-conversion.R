...
.get_idx_of_loops_from_dbs <- function(dbs,
                                       type,
                                       warn.type.drops){
...
.get_idx_of_loops_from_dbdfl <- function(dbdfl,
                                         type,
                                         warn.type.drops){
...
setMethod("getLoopIndices",
          ...
          definition = function(x,
                                bracket.type,
                                warn.type.drops = TRUE){
            .get_idx_of_loops_from_dbdfl(SDBDFL(x),
                                         bracket.type,
                                         warn.type.drops)[[1L]]
          })
...
.str_replace_all_custom_c <- function(string,
                                      pattern,
                                      replacement){
...
.convert_db_annotation <- function(x,
                                   from,
                                   to){
  if(missing(from) ||
     missing(to) ||
...
setMethod("convertAnnotation",
          ...
          definition = function(x,
                                from,
                                to){
            .convert_db_annotation(as(x,"DotBracketStringSet"),
                                   from,
                                   to)[[1]]
          })
#' @rdname convertAnnotation
...
setMethod("convertAnnotation",
          ...
          definition = function(x,
                                from,
                                to){
            DotBracketStringSetList(lapply(x, .convert_db_annotation,
                                           from,
                                           to))
          })
